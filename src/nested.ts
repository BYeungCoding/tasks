...
export function toCSV(questions: Question[]): string {
    ...
    const lines = questions.map((q) => {
        ...
        const name = q.name;
        const options = q.options.length.toString();
        const points = q.points.toString();
        const published = q.published.toString();
        return [id, name, options, points, published].join(",");
    });
...
export function sameType(questions: Question[]): boolean {
    ...
    const referenceType = questions[0].type;
    return questions.every((question) => question.type === referenceType);
}
